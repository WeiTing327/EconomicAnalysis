...
def ffill_tensor(tensor_input, method=1):
    ...
    if method == 2:
        ...
        for k in range(1, tensor_input.shape[0]):
            tensor_input[:, :, k] = torch.where(
                mask[:, :, k], tensor_input[:, :, k - 1], tensor_input[:, :, k]
            )

    if 1:  # 把未來資料變成nan 這樣避免以後的問題 # 做成下三角矩陣
        i_indices = torch.arange(tensor_input.shape[0]).view(
            -1, 1, 1
        )  # 形狀為 [i, 1, 1]
        k_indices = torch.arange(tensor_input.shape[2]).view(
            1, 1, -1
        )  # 形狀為 [1, 1, k]
        mask = (
            k_indices > i_indices
        )  # 形狀為 [i, 1, k] 建立一個遮罩來標記所有 k > i 的位置
        mask = mask.expand(
            tensor_input.shape[0], tensor_input.shape[1], -1
        )  # 將遮罩擴展
        # 使用遮罩將符合條件的位置填充為 NaN
...
def create_tensor(symbol_files, data_path="", save_path=""):
    ...
    for symbol_idx, file in enumerate(symbol_files):
        ...
        df["realtime_end"] = pd.to_datetime(
            df["realtime_end"], errors="coerce"
        )  # 全部都是9999-12-31會print錯誤但不影響
        ...
        max_date = max(
            [df["date"].max(), df["realtime_start"].max(), df["realtime_end"].max()]
        )
        min_date = min(
            [df["date"].min(), df["realtime_start"].min(), df["realtime_end"].min()]
        )
        # 建立日期範圍
        date_list = pd.date_range(start=min_date, end=max_date)
        print(
            f"max_date:{max_date} | min_date:{min_date} |  df.shape {df.shape} | file {file}"
        )
        is_daily = False
        ...
        if (
            not is_daily and (df["realtime_start"][0] - df["date"][0]).days > 500
        ):  # 很晚才新增資料 才會有這麼大的差距 就採用自動修正
            ...
            if 1:  # 修正非日資料但是又很晚(ex .2024) 才加入資料庫 ex(CPILFESL)
                check = df.groupby("date").first()
                check = check.reset_index()
                check["diff"] = check["realtime_start"] - check["date"]

                adj_realtime_start_date = check.iloc[-1][
                    "diff"
                ].days  # 用最後一筆判斷資料會遞延多久
                # 直接使用布林條件篩選符合條件的行
                condition = (
                    check["realtime_start"] - check["date"]
                ).dt.days < -adj_realtime_start_date
                print(f"symbol {file} 在資料日後{adj_realtime_start_date}公布資料")
                # 新增的資料計算
                new_realtime_start = check.loc[condition, "date"] + timedelta(
                    days=adj_realtime_start_date
                )
                new_realtime_end = new_realtime_start - timedelta(days=1)

                new_realtime_start = new_realtime_start.apply(
                    lambda x: min(x, max_date)
                )  # 確定不超過today
                new_realtime_end = new_realtime_end.apply(
                    lambda x: min(x, max_date)
                )  # 確定不超過today
        ...
        if 1:
            ...
            date_tensor = torch.full(
                (len(date_list), 1, len(date_list)), np.nan, dtype=torch.float64
            )
        ...
        for _, row in df.iterrows():
            # print(is_daily)
            if is_daily:
                start_pos = row["date_position"]  # 日資料
            else:
                start_pos = row["realtime_start_position"]
                # start_pos = row['realtime_end_position']
            # start_pos = row['realtime_start_position']
            ...
            if 1:
                ...
                date_tensor[start_pos : (end_pos + 1), 0, date_pos] = (
                    int(datadate.strftime("%Y%m%d")) * 0.0001
                )  # 轉八碼填載小數位
                fillmethod = 1
            if 0:
                # 將指定範圍內的值填入 tensor
                tensor[date_pos, 0, start_pos : (end_pos + 1)] = value  # 填值
                date_tensor[date_pos, 0, start_pos : (end_pos + 1)] = (
                    int(datadate.strftime("%Y%m%d")) * 0.0001
                )  # 轉八碼填載小數位
                fillmethod = 2

        # print(f"開始filled_tensor | symbol_idx {symbol_idx} | file {file} |time {datetime.now()}")
        filled_tensor = ffill_tensor(tensor.to(device), method=fillmethod).to(
            "cpu"
        )  # [:,0,:] 會是一個下三角的矩陣 但不會是從第一筆開始
        # print(f"開始filled_tensor_date | symbol_idx {symbol_idx} | file {file} |time {datetime.now()}")
        filled_tensor_date = ffill_tensor(date_tensor.to(device), method=fillmethod).to(
            "cpu"
        )  # [:,0,:] 會是一個下三角的矩陣 但不會是從第一筆開始
        ...
        save_tensor(
            filled_tensor,
            os.path.join(save_path, file.split(".")[0], file.split(".")[0] + ".pt"),
            method="hdf5",
        )
        save_tensor(
            filled_tensor_date,
            os.path.join(
                save_path, file.split(".")[0], file.split(".")[0] + "_date.pt"
            ),
            method="hdf5",
        )
        save_pickle(
            date_list,
            os.path.join(
                save_path, file.split(".")[0], file.split(".")[0] + "_date_list.pkl"
            ),
        )
        pd.DataFrame(date_list, columns=["date"]).to_csv(
            os.path.join(
                save_path, file.split(".")[0], file.split(".")[0] + "_date_list.csv"
            ),
            index=False,
            encoding="utf-8-sig",
        )

        # pd.DataFrame(date_list, columns=["Date"]).to_csv(os.path.join(save_path, file.split(".")[0] + '_date_list.csv'),index=False,encoding='utf-8-sig')
        ...
        if 0:
            ...
            output_path = os.path.join(
                save_path, file.split(".")[0], file.split(".")[0] + ".csv"
            )
        ...
        if 1:
            output_path = os.path.join(
                save_path,
                file.split(".")[0],
                file.split(".")[0] + "_raw_data_自動修正後.csv",
            )
...
if __name__ == "__main__":
    ...
    # symbol_files_list = [symbol_files_list [-2]]
    # symbol_files_list = ['UNRATE.csv']
    # UNRATE PAYEMS需另外確認
    if 1:  # 製作GDPC1.pt
