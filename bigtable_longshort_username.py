...
class BuildBigtable:
    ...
    def build(df, rule_list):
        ...
        for rule_name in rule_list:
            ...
            tensor = load_tensor(
                os.path.join(rule_path, rule_name + ".pt"), method="hdf5"
            )
            date_list = load_pickle(
                os.path.join(
                    measure_path, measure_name, measure_name + "_date_list.pkl"
                )
            )
        ...
        long_df = pd.DataFrame(long_results_list).sort_values(
            by="precision", ascending=False
        )
        short_df = pd.DataFrame(short_results_list).sort_values(
            by="precision", ascending=False
        )
