...
def signal_count_check_input_command(operationn, **variables):
    ...
    print(
        f"訊號出現 {count} 次 總共用 {result.shape[0]} 天 | 比例為 {(count / result.shape[0] * 100):.2f}%"
    )
...
def diff_track_adjusted_values_change(tensor, windows=60):
    ...
    diff = torch.cat(
        (
            torch.zeros(windows, tensor.size(1)),
            (
                get_diagonal_values(tensor)[windows:]
                - get_diagonal_values(tensor)[:-windows]
            ),
        ),
        dim=0,
...
def diff_record_daily_value_change(tensor, windows=60):
    ...
    diff = torch.cat(
        (
            torch.zeros((tensor.size(0), tensor.size(1), windows)),
            tensor[:, :, windows:] - tensor[:, :, :-windows],
        ),
        dim=2,
    )
...
def percent_change_record_daily_value_change(tensor, windows=60):
    ...
    percent_change = (current_values - past_values) / (past_values + 1e-8)
    diff = torch.cat(
        (torch.zeros((tensor.size(0), tensor.size(1), windows)), percent_change), dim=2
    )
...
def rule_calculate(operation, **variables):
    ...
    result = (
        eval(operation, globals(), variables) * 1
    )  # 使用空的全局和局部變數字典來避免潛在的安全問題
