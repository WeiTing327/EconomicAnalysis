...
class RuleCombiner:
    ...
    def load_data(self, file_path):
        try:
            ...
            selected_columns = ['measure', 'rule', 'rule_describe', 'accuracy', 'precision', 'recall', 'f1_score']
            sample_data = df[selected_columns]

            # 提取 'rule' 中的編號並更新 'rule' 欄位
            sample_data['rule_number'] = sample_data['rule'].str.extract(r'(\d+)').astype(int)
    ...
    def generate_combinations(self, df):
        ...
        for _ in range(self.num_combinations):
            ...
            # 按照 rule 編號排序
            sample_rules = sample_rules.sort_values(by='rule', key=lambda x: x.str.extract(r'(\d+)', expand=False).astype(int))
